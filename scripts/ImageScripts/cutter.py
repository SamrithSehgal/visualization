...
#Source: https://github.com/Ailyth99/leaflet-tile-cutter
img_path=""
max_zoom_level=4
output='output'
...
def cut_tiles(image_path, max_zoom_level, output_folder, imgType):
    ...
    for zoom_level, info in zoom_levels.items():
        ...
        for x in range(0, zoomed_size[0], tile_size[0]):
            ...
            for y in range(0, zoomed_size[1], tile_size[1]):
                ...
                tile.save(f"{dir_path}/{y//tile_size[1]}.{imgType}")
                tileByteArray = io.BytesIO()
                tile.save(tileByteArray, format="PNG")
                curTiles.append(tileByteArray.getvalue())
            
            curZoom.append(curTiles)
        tileBinaries.append(curZoom)
