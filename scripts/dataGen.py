...
for h in range(len(days)):
    for i in range(len(buildings)):
        for x in range(numFloors[i]):
            occupancy = random.randint(5000, 6000)

            numsElement = driver.find_element(By.ID, "numbers")
            lowElement = driver.find_element(By.ID, "low")
            highElement = driver.find_element(By.ID, "high")

            lowElement.clear()
            lowElement.send_keys(str(round(occupancy/10)))

            highElement.clear()
            highElement.send_keys(str(round(occupancy/3)))
            ...
            lowElement.clear()  
            lowElement.send_keys("7")
            highElement.clear()
            highElement.send_keys("93")
